 ...
 *   → Deploys new implementation with RELAYER_ROLE, schedules upgrade via timelock.
 *
 * Phase 2 (EXECUTE — after 24 h delay):
 *   PHASE=execute npx hardhat run scripts/upgrade-bridge-relayer-role.ts --network sepolia
 *   → Executes the scheduled upgrade, verifies state, grants RELAYER_ROLE to relay EOA.
 *
...
const SALT         = ethers.id("upgrade-bridge-relayer-role-2026-02-17");
...
async function scheduleUpgrade(
  ...
  console.log("\n═══ PHASE 1: SCHEDULE (BRIDGE-M-04 RELAYER_ROLE) ═══\n");
...
async function executeUpgrade(deployer: any, timelock: any) {
  console.log("\n═══ PHASE 2: EXECUTE (BRIDGE-M-04 RELAYER_ROLE) ═══\n");
  ...
  // Verify RELAYER_ROLE exists
  const RELAYER_ROLE = await bridge.RELAYER_ROLE();
  console.log("\n✅ RELAYER_ROLE:", RELAYER_ROLE);

  // Grant RELAYER_ROLE to relay EOA
  console.log(`\n⚙️  Granting RELAYER_ROLE to relay EOA ${RELAY_EOA}...`);
  const hasRole = await bridge.hasRole(RELAYER_ROLE, RELAY_EOA);
  if (hasRole) {
    console.log("✅ Relay EOA already has RELAYER_ROLE (granted in initialize)");
  } else {
    const grantTx = await bridge.grantRole(RELAYER_ROLE, RELAY_EOA);
    await grantTx.wait();
    console.log("✅ RELAYER_ROLE granted to relay EOA");
  }

  // Verify
  const confirmed = await bridge.hasRole(RELAYER_ROLE, RELAY_EOA);
  if (!confirmed) throw new Error("Failed to grant RELAYER_ROLE to relay EOA!");
  console.log("✅ Confirmed: relay EOA has RELAYER_ROLE");

  // Also verify deployer has RELAYER_ROLE (from initialize)
