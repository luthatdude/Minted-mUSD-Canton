...
MAX_TOKENS = 4096  # FIX: was referenced in ai_analyze() but never defined
...
def _api_get(url: str, timeout: int = API_TIMEOUT) -> requests.Response:
    """GET with retry logic for DefiLlama API resilience."""
    import time
    last_err = None
...
class DefiScanner:
    ...
    def fetch_all(self, force=False):
        ...
        with Progress(
            ...
            try:
                resp2 = _api_get(DEFILLAMA_LEND_BORROW)
                lb_data = resp2.json()
                for item in lb_data:
    ...
    def calculate_looping(
        ...
        for loops in range(1, max_loops + 1):
            ...
            # FIX: Proper liquidation buffer calculation
            # After N loops, total debt / total collateral = ltv^loops * ltv
            # Health factor = (collateral * liquidation_threshold) / debt
            # Buffer = how far price can drop before liquidation
            total_collateral_ratio = leverage          # sum(ltv^0..N)
            total_debt_ratio = leverage - 1            # sum(ltv^1..N)
            if total_collateral_ratio > 0 and total_debt_ratio > 0:
                effective_ltv_used = total_debt_ratio / total_collateral_ratio
                # Buffer = distance from effective LTV to liquidation LTV
                liq_buffer = ltv - effective_ltv_used
            else:
                liq_buffer = ltv
    ...
    def find_borrow_lend_arb(self, min_spread: float = 1.0, min_tvl: float = 5_000_000) -> list[dict]:
        ...
        # Group by symbol
        by_symbol: dict[str, list[YieldPool]] = {}
            ...
            for bp in borrow_pools:
                for sp in supply_pools:
                    ...
                    if spread >= min_spread:
                        # FIX: flag cross-chain arbs that require bridging
                        cross_chain = bp.chain != sp.chain
                        arbs.append({
                            "symbol": sym,
...
def display_looping_table(strategies: list[LoopingStrategy]):
    ...
    for strat in strategies[:30]:
        table.add_row(
            strat.pool.project,
            strat.pool.chain_short,
            strat.pool.symbol[:16],
            fmt_pct(strat.supply_apy),
            str(strat.loops),
            f"{strat.leverage:.1f}x",
            f"[bold green]{fmt_pct(strat.net_apy)}[/bold green]",
            fmt_pct(strat.borrow_apy),
            f"{strat.ltv:.0%}" if strat.ltv else "—",
            fmt_usd(strat.pool.tvl),
            strat.pool.risk_label,
        )
...
def display_arb_table(arbs: list[dict]):
    ...
    table.add_column("Bridge", style="dim")

    for a in arbs[:25]:
        risk_score = a["risk"]
        risk_label = "🟢" if risk_score <= 3 else "🟡" if risk_score <= 5 else "🔴"
        # FIX: flag cross-chain arbs that need bridging
        bridge_flag = "⚠️ BRIDGE" if a.get("cross_chain") else "same"
        table.add_row(
            a["symbol"],
...
[cyan]Total Pools:[/cyan]  {summary['total_pools']:,}  across  [cyan]{summary['chains']}[/cyan] chains  and  [cyan]{summary['protocols']}[/cyan] protocols
[cyan]Total TVL:[/cyan]    {fmt_usd(summary['total_tvl'])}

[bold]Stablecoins[/bold]
  Pools: {summary['stable_pools']:,}   TVL: {fmt_usd(summary['stable_tvl'])}
  Avg Yield: [green]{summary['stable_avg_yield']:.2f}%[/green]   Median: [green]{summary['stable_median_yield']:.2f}%[/green]

[bold]Lending[/bold]
  Avg Supply Rate: [green]{summary['lending_avg_yield']:.2f}%[/green]

[bold]LP Pools:[/bold]  {summary['lp_pools']:,}""",
        title="◈ Market Scan",
        border_style="green",
        padding=(1, 2),
    ))
...
def generate_report(scanner: DefiScanner, output_path: str = "defi_report.md"):
    ...
    lines.append(f"Scanned {summary['total_pools']:,} pools across {summary['chains']} chains and {summary['protocols']} protocols.\n")
    lines.append(f"Total TVL tracked: {fmt_usd(summary['total_tvl'])}\n")
    ...
    lines.append("|----------|-------|-------|----------|-------|---------|----------|")
    loop_strats = scanner.find_looping_opportunities(min_tvl=5_000_000)
    for strat in loop_strats[:15]:
        lines.append(f"| {strat.pool.project} | {strat.pool.chain_short} | {strat.pool.symbol} | {fmt_pct(strat.supply_apy)} | {strat.loops} | {fmt_pct(strat.net_apy)} | {strat.leverage:.1f}x |")

    # Arbitrage
    lines.append("\n## Borrow/Lend Arbitrage\n")
    lines.append("| Asset | Borrow From | Rate | Lend To | Rate | Spread | Bridge |")
    lines.append("|-------|-------------|------|---------|------|--------|--------|")
    arbs = scanner.find_borrow_lend_arb(min_spread=0.5)
    for a in arbs[:15]:
        bridge = "⚠️" if a.get("cross_chain") else "—"
        lines.append(f"| {a['symbol']} | {a['borrow_from']} ({a['borrow_chain']}) | {fmt_pct(a['borrow_rate'])} | {a['lend_to']} ({a['lend_chain']}) | {fmt_pct(a['lend_rate'])} | {fmt_pct(a['spread'])} | {bridge} |")
...
def interactive(scanner: DefiScanner):
    ...
    while True:
        ...
        if action in ("quit", "exit", "q"):
            ...
            # Also show lending
            lending_pools = [p for p in pools if p.borrow_rate is not None]
            if lending_pools:
                display_lending_table(lending_pools, f"{symbol} Lending/Borrowing")
