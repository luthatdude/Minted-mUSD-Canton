...
contract PriceOracle is AccessControl {
    ...
    function keeperResetPrice(address token) external onlyRole(KEEPER_ROLE) {
        ...
        FeedConfig storage config = feeds[token];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010001,0)}
        if (!config.enabled) revert FeedNotEnabled();
        (, int256 answer, , uint256 updatedAt, ) = config.feed.latestRoundData();assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010002,0)}
        if (answer <= 0) revert InvalidPrice();
        if (block.timestamp - updatedAt > config.stalePeriod) revert StalePrice();
        
        uint8 feedDecimals = config.feed.decimals();assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000003,feedDecimals)}
        uint256 newPrice = uint256(answer) * (10 ** (18 - feedDecimals));assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000004,newPrice)}
    ...
    function resetLastKnownPrice(address token) external onlyRole(ORACLE_ADMIN_ROLE) {
        FeedConfig storage config = feeds[token];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010005,0)}
        if (!config.enabled) revert FeedNotEnabled();
        (, int256 answer, , , ) = config.feed.latestRoundData();assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010006,0)}
        if (answer <= 0) revert InvalidPrice();
        uint8 feedDecimals = config.feed.decimals();assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000007,feedDecimals)}
    ...
    ///      GAS-01 optimization.
    function _getPrice(address token) internal view returns (uint256 price) {assembly ("memory-safe") { mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00000000, 1037618708480) mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00000001, 1) mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00000005, 1) mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00006000, token) }
        FeedConfig storage config = feeds[token];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010008,0)}
        if (!config.enabled) revert FeedNotEnabled();
        ...
        (
            ...
            uint256 updatedAt,
            uint80 answeredInRound
        ) = config.feed.latestRoundData();assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010009,0)}

        if (answer <= 0) revert InvalidPrice();
        if (block.timestamp - updatedAt > config.stalePeriod) revert StalePrice();
        if (answeredInRound < roundId) revert StaleRound();

        uint8 feedDecimals = config.feed.decimals();assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000000a,feedDecimals)}
        ...
        price = uint256(answer) * (10 ** (18 - feedDecimals));assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000001a,price)}
    ...
    function updatePrice(address token) external onlyRole(ORACLE_ADMIN_ROLE) {
        FeedConfig storage config = feeds[token];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0001000b,0)}
        if (!config.enabled) revert FeedNotEnabled();
        
        (, int256 answer, , uint256 updatedAt, ) = config.feed.latestRoundData();assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0001000c,0)}
        if (answer <= 0) revert InvalidPrice();
        if (block.timestamp - updatedAt > config.stalePeriod) revert StalePrice();
        
        uint8 feedDecimals = config.feed.decimals();assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000000d,feedDecimals)}
        uint256 newPrice = uint256(answer) * (10 ** (18 - feedDecimals));assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000000e,newPrice)}
        uint256 oldPrice = lastKnownPrice[token];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000000f,oldPrice)}
    ...
    function getValueUsd(address token, uint256 amount) external view returns (uint256 valueUsd) {
        uint256 priceNormalized = _getPrice(token);assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000010,priceNormalized)}
        valueUsd = (amount * priceNormalized) / (10 ** feeds[token].tokenDecimals);assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000001b,valueUsd)}
    }
    ...
    function _getPriceUnsafe(address token) internal view returns (uint256 price) {assembly ("memory-safe") { mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00010000, 1037618708481) mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00010001, 1) mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00010005, 1) mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00016000, token) }
        FeedConfig storage config = feeds[token];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010011,0)}
        if (!config.enabled) revert FeedNotEnabled();

        (uint80 roundId, int256 answer, , uint256 updatedAt, uint80 answeredInRound) = config.feed.latestRoundData();assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010012,0)}
        if (answer <= 0) revert InvalidPrice();
        if (block.timestamp - updatedAt > config.stalePeriod) revert StalePrice();
        if (answeredInRound < roundId) revert StaleRound();

        uint8 feedDecimals = config.feed.decimals();assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000013,feedDecimals)}
        if (feedDecimals > 18) revert UnsupportedFeedDecimals();
        price = uint256(answer) * (10 ** (18 - feedDecimals));assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000001c,price)}
    ...
    function getValueUsdUnsafe(address token, uint256 amount) external view returns (uint256 valueUsd) {
        uint256 priceNormalized = _getPriceUnsafe(token);assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000014,priceNormalized)}
        valueUsd = (amount * priceNormalized) / (10 ** feeds[token].tokenDecimals);assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000001d,valueUsd)}
    }
    ...
    function refreshPrice(address token) external {
        ...
        FeedConfig storage config = feeds[token];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010015,0)}
        if (!config.enabled) revert FeedNotEnabled();

        (, int256 answer, , uint256 updatedAt, ) = config.feed.latestRoundData();assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010016,0)}
        if (answer <= 0) revert InvalidPrice();
        if (block.timestamp - updatedAt > config.stalePeriod) revert StalePrice();

        uint8 feedDecimals = config.feed.decimals();assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000017,feedDecimals)}
        uint256 newPrice = uint256(answer) * (10 ** (18 - feedDecimals));assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000018,newPrice)}
    ...
    function isFeedHealthy(address token) external view returns (bool) {
        FeedConfig storage config = feeds[token];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010019,0)}
