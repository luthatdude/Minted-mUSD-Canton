...
contract DepositRouter is AccessControl, ReentrancyGuard, Pausable {
    ...
    function previewDeposit(uint256 amount) external view returns (uint256 netAmount, uint256 fee) {
        fee = (amount * feeBps) / 10000;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000000c,fee)}
        netAmount = amount - fee;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000000d,netAmount)}
    }
     ...
     */
    function quoteBridgeCost() public view returns (uint256 nativeCost) {assembly ("memory-safe") { mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00010000, 1037618708481) mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00010001, 0) mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00010004, 0) }
        (nativeCost, ) = wormholeRelayer.quoteEVMDeliveryPrice(
            ETHEREUM_CHAIN_ID,
            0, // No native token to send
            GAS_LIMIT
        );assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0002000e,0)}
    }
    ...
    function markDepositComplete(uint64 sequence) external onlyRole(ROUTER_ADMIN_ROLE) {
        PendingDeposit storage pendingDep = pendingDeposits[sequence];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010001,0)}
        if (pendingDep.depositor == address(0)) revert DepositNotFound();
        if (pendingDep.completed) revert AlreadyCompleted();
        pendingDep.completed = true;bool certora_local15 = pendingDep.completed;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000000f,certora_local15)}
    ...
    function markDepositsComplete(uint64[] calldata sequences) external onlyRole(ROUTER_ADMIN_ROLE) {
        for (uint256 i = 0; i < sequences.length; i++) {
            PendingDeposit storage pendingDep = pendingDeposits[sequences[i]];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010011,0)}
    ...
    function setTreasury(address newTreasury) external onlyRole(TIMELOCK_ROLE) {
        ...
        address old = treasuryAddress;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000002,old)}
    ...
    function setDirectMint(address newDirectMint) external onlyRole(TIMELOCK_ROLE) {
        ...
        address old = directMintAddress;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000003,old)}
    ...
    function setFee(uint256 newFeeBps) external onlyRole(TIMELOCK_ROLE) {
        ...
        uint256 old = feeBps;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000004,old)}
        feeBps = newFeeBps;
    ...
    function withdrawFees(address to) external onlyRole(ROUTER_ADMIN_ROLE) {
        ...
        uint256 amount = accumulatedFees;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000005,amount)}
        accumulatedFees = 0;
    ...
    function emergencyWithdraw(address token, uint256 amount) external onlyRole(TIMELOCK_ROLE) whenPaused {
        ...
        address to = treasuryAddress;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000006,to)}
        if (token == address(0)) {
    ...
    function _deposit(address depositor, uint256 amount) internal returns (uint64 sequence) {assembly ("memory-safe") { mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00000000, 1037618708480) mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00000001, 2) mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00000005, 9) mstore(0xffffff6e4604afefe123321beef1b01fffffffffffffffffffffffff00006001, amount) }
        ...
        // Calculate fee
        uint256 fee = (amount * feeBps) / 10000;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000007,fee)}
        uint256 netAmount = amount - fee;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000008,netAmount)}
        
        // Get bridge cost quote
        uint256 bridgeCost = quoteBridgeCost();assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000009,bridgeCost)}
        ...
        bytes32 treasuryReceiverBytes = bytes32(uint256(uint160(treasuryAddress)));assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000000a,treasuryReceiverBytes)}
        
        // knows who to mint mUSD for. Without this, TreasuryReceiver's
        // abi.decode(vm.payload, (address)) would fail on empty payload.
        bytes memory recipientPayload = abi.encode(depositor);assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0001000b,0)}
        ...
        sequence = tokenBridge.transferTokensWithPayload{value: bridgeCost}(
            ...
            _nonce,
            recipientPayload
        );assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000010,sequence)}
