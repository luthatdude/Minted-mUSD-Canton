...
const stakeLog = new IdempotencyStore<StakeRecord>();
...
export default async function handler(
  ...
  try {
    ...
    if (!gte(totalCip56, stakeAmount)) {
      return res.status(400).json({
        error: `Insufficient CIP-56 balance: have ${toDisplay(totalCip56)}, need ${toDisplay(stakeAmount)}`,
        mode: "native",
      });
    }
    ...
    for (const c of userCip56) {
      if (gte(selectedSum, stakeAmount)) break;
    ...
    const idemKey = deriveIdempotencyKey("stake", sourceCids, toDisplay(stakeAmount), userParty);
    ...
    if (!gte(totalOperatorMusd, stakeAmount)) {
      return res.status(409).json({
        error: `Insufficient operator inventory: have ${toDisplay(totalOperatorMusd)}, need ${toDisplay(stakeAmount)}`,
    ...
    for (const c of operatorMusd) {
      if (gte(inventorySum, stakeAmount)) break;
    ...
    commands.push({
      CreateCommand: {
        ...
        createArguments: {
          issuer: cip56Issuer,
          owner: operatorParty,
          amount: toDamlDecimal(stakeAmount),
          blacklisted: false,
          agreementHash: refCip56.agreementHash,
          agreementUri: refCip56.agreementUri,
          observers: [],
    ...
    commands.push({
      ExerciseCommand: {
        ...
        choiceArgument: {
          ...
          stakeAmount: toDamlDecimal(stakeAmount),
        },
      },
    });
    ...
    console.log(
      ...
      `for ${toDisplay(stakeAmount)} mUSD, user=${userParty.slice(0, 30)}`
    );
    ...
    const record: StakeRecord = {
      success: true,
      mode: "native",
      stakeAmount: toDisplay(stakeAmount),
      commandId,
