...
async function main() {
  console.log(`[topup] target=${TARGET} chunk=${CHUNK} max-tx=${MAX_TX} execute=${EXECUTE}`);
  ...
  for (let i = 1; i <= txCount; i++) {
    ...
    try {
      ...
      failed++;
      console.error(`  tx${i}: FAIL — ${err.message?.slice(0, 100)}`);
    }
  }
  ...
  try {
    ...
    const afterInv = parseFloat(after.operatorInventory);
    ...
    console.error(`[topup] Post-check failed: ${err.message}`);
    process.exit(1);
  }
}
