 ...
 *
 * YIELD MODEL CLARIFICATION
 * Collateral in Morpho Blue earns 0% supply rate.
 * Strategy yield comes from Morpho token incentives/rewards on the market.
 ...
 UUPSUpgradeable
{
 ...
 // Validate all addresses to prevent bricked proxy
 ...
 */
 ...
 // Use actual freed amount, not requested, to prevent principal desync
 ...
 */
 ...
 // Return and emit actual transferred amount, not deleverage return
 withdrawn = balance;
 ...
 */
 ...
 // Include idle USDC balance in total value.
 ...
 * @dev Now checks profitability before looping
 ...
 */
 ...
 // WARNING — collateral earns 0% interest in Morpho Blue.
 ...
 // supplyCollateral earns 0% in Morpho Blue — strategy yield comes
 ...
 * @dev Return actual freed amount, not inflated collateral withdrawn
 ...
 * @dev Return actual freed amount based on ending balance
 */
 function _fullDeleverage() internal returns (uint256 totalFreed) {
 // Track starting balance to calculate actual net freed
 uint256 startingBalance = usdc.balanceOf(address(this));
 
 // Increase loop count from MAX_LOOPS*2 to MAX_LOOPS*3 to ensure full deleverage
 ...
 // Don't count here - calculate at end
 }
 break;
 }
 ...
 if (balance > 0) {
 uint256 repayAmount = balance > currentBorrow ? currentBorrow : balance;
 // Per-operation approval for _fullDeleverage repay
 usdc.forceApprove(address(morpho), repayAmount);
 morpho.repay(marketParams, repayAmount, 0, address(this), "");
 }
 }
 ...
 */
 ...
 // Removed WAD/WAD no-op (was dividing and multiplying by WAD)
 ...
 */
 ...
 // Prevent underflow in _maxWithdrawable() if buffer >= LTV
 ...
 * @dev Allows strategist to tune when looping is profitable
 ...
 */
 ...
 // Deactivate and pause to prevent next deposit from re-leveraging
 active = false;
 _pause();
