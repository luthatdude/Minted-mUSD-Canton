...
import "../interfaces/ISkyPSM.sol";
import "../interfaces/ISUSDSVault.sol";
 ...
 UUPSUpgradeable
{
 ...
 // Removed infinite approvals. Using per-operation forceApprove instead
 ...
 // When tout > 0, PSM deducts extra USDS. Reduce gemAmt to avoid revert.
 ...
 // Always include idle USDC, even when shares == 0
 ...
 /// Recipient must hold TREASURY_ROLE to prevent compromised guardian
 ...
 if (usdsBalance > 0) {
 // Account for PSM tout fee to avoid revert
 uint256 tout = psm.tout();
 uint256 usdcToReceive;
 if (tout > 0) {
 ...
 if (usdcToReceive > 0) {
 // Approve USDS for PSM before conversion
