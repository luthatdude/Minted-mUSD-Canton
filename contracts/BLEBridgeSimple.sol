...
// BLE Protocol - Simple Bridge Mode
// Storage-layout compatible with BLEBridgeV9 for UUPS upgrade path
//
// ╔═══════════════════════════════════════════════════════════════════════════════╗
// ║  STORAGE NOTE                                                                   ║
// ║                                                                               ║
// ║  This contract intentionally preserves BLEBridgeV9 storage slot ordering.     ║
// ║  Do not reorder, remove, or insert state variables.                           ║
// ║                                                                               ║
// ║  V9/Simple Storage Layout (12 vars + __gap[38] = 50 slots):                  ║
...
contract BLEBridgeSimple is Initializable, AccessControlUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    /// @notice Simple-mode update path used by the relay.
    /// @dev RELAYER_ROLE is the trust anchor in simple mode.
    function updateCantonAssets(uint256 cantonAssets)
        external
        nonReentrant
        whenNotPaused
        ...
        returns (uint256 newCap)
    {
        ...
        bytes32 entropy = keccak256(
            abi.encodePacked(address(this), msg.sender, block.number, nonce, cantonAssets, timestamp)
        );
        bytes32 id = computeAttestationId(nonce, cantonAssets, timestamp, entropy, bytes32(0));
        newCap = _applySimpleUpdate(id, cantonAssets, nonce, timestamp, entropy, bytes32(0));

        emit AttestationReceived(id, cantonAssets, newCap, nonce, timestamp);
    }

    /// @notice Compatibility shim for existing relayer payloads.
    /// @dev Signatures are accepted for interface compatibility but ignored in simple mode.
    function processAttestation(
        ...
        uint256 nonce = att.nonce == 0 ? currentNonce + 1 : att.nonce;
        uint256 timestamp = att.timestamp == 0 ? block.timestamp : att.timestamp;
        bytes32 entropy = att.entropy == bytes32(0)
            ? keccak256(abi.encodePacked(address(this), msg.sender, block.number, nonce, att.cantonAssets, timestamp))
            : att.entropy;
        bytes32 id = att.id == bytes32(0)
            ? computeAttestationId(nonce, att.cantonAssets, timestamp, entropy, att.cantonStateHash)
            : att.id;

        uint256 newCap = _applySimpleUpdate(id, att.cantonAssets, nonce, timestamp, entropy, att.cantonStateHash);
        emit AttestationReceived(id, att.cantonAssets, newCap, nonce, timestamp);
    }

    function _applySimpleUpdate(
        bytes32 id,
        uint256 cantonAssets,
        uint256 nonce,
        uint256 timestamp,
    ...
    ) internal returns (uint256 newCap) {
        if (nonce != currentNonce + 1) revert InvalidNonce();
        if (usedAttestationIds[id]) revert AttestationReused();
        if (cantonAssets == 0) revert ZeroAssets();
        if (timestamp > block.timestamp) revert FutureTimestamp();
        if (timestamp < lastAttestationTime + MIN_ATTESTATION_GAP) revert AttestationTooClose();
        if (block.timestamp - timestamp > MAX_ATTESTATION_AGE) revert AttestationTooOld();
        if (entropy == bytes32(0)) revert MissingEntropy();

        bytes32 expectedId = computeAttestationId(nonce, cantonAssets, timestamp, entropy, cantonStateHash);
        if (id != expectedId) revert InvalidAttestationId();

        usedAttestationIds[id] = true;
        currentNonce = nonce;
        lastAttestationTime = timestamp;
        attestedCantonAssets = cantonAssets;

        if (cantonStateHash != bytes32(0)) {
            verifiedStateHashes[cantonStateHash] = true;
            lastCantonStateHash = cantonStateHash;
            emit CantonStateHashVerified(cantonStateHash, id);
        }

        newCap = _updateSupplyCap(cantonAssets, false);
    }
