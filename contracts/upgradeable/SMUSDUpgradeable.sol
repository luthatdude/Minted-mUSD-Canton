...
contract SMUSDUpgradeable is ERC4626Upgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, UUPSUpgradeable {
 ...
 /// @notice Cached last known good globalTotalAssets to prevent
 ...
 // Added nonReentrant and whenNotPaused for security
 // Ensure globalTotalAssets cache is populated before accepting deposits.
 ...
 // Added maximum yield cap to prevent excessive dilution attacks
 ...
 // Track for analytics
 // Cap to prevent overflow on totalInterestReceived
 unchecked {
 ...
 /// @notice Set the treasury address — must be called through MintedTimelockController
 ...
 // When no ETH shares exist, cap at absolute limit
 ...
 /// @dev No longer silently falls back to local totalAssets.
 ...
 /// @dev Uses typed interface call instead of raw staticcall for
 ...
 // Clamp growth to MAX_GLOBAL_ASSETS_GROWTH_BPS per refresh.
 ...
 /// @dev Guard against calling when no shares exist (division-by-zero in downstream callers)
 /// @dev Restricted to YIELD_MANAGER_ROLE to prevent cache manipulation attacks
 ...
 /// @dev Delegates to internal _convertToShares to ensure preview
 ...
 /// @dev Delegates to internal _convertToAssets to ensure preview
 ...
 // ============================================================
 // maxWithdraw/maxDeposit/maxMint/maxRedeem
 // Must use globalTotalAssets for consistency with _convertToShares
 // ============================================================

 /// @notice Cap at local mUSD balance for ERC-4626 compliance.
 ...
 /// @notice Max redeemable shares for owner, capped by vault's local mUSD balance.
