...
contract BLEBridgeV9 is Initializable, AccessControlUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
 ...
 /// @notice Role for authorized relayers to submit attestations
 ...
 // unpauseRequestTime moved here from mid-contract
 // to prevent storage layout fragility on future UUPS upgrades.
 uint256 public unpauseRequestTime;

 /// Maximum attestation age — reject attestations older than this
 uint256 public constant MAX_ATTESTATION_AGE = 6 hours;
 
 /// Minimum gap between attestation timestamps to prevent same-block replay
 ...
 /// Execute unpause after timelock delay
 ...
 /// Kept for interface compatibility but reverts with guidance
 ...
 /// @dev In normal mode, cap cannot go below totalSupply.
 ...
 // Allow going below totalSupply only with admin authority
 ...
 /// Added reason parameter and event emission for audit trail
 ...
 /// @dev Restricted to RELAYER_ROLE to prevent griefing via rate-limit exhaustion
 ...
 /// Require minimum gap between attestation timestamps
 require(att.timestamp >= lastAttestationTime + MIN_ATTESTATION_GAP, "ATTESTATION_TOO_CLOSE");
 /// Reject attestations older than MAX_ATTESTATION_AGE
 ...
 /// Use >= to prevent boundary timing attack at exact reset second
