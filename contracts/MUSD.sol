...
/// Add Pausable so transfers can be halted in emergencies
contract MUSD is ERC20, AccessControl, Pausable {
 ...
 /// @dev Allow cap decreases for undercollateralization response.
 ...
 // Prevent blacklisting protocol-critical addresses
 if (status) {
 ...
 /// Added zero-address check to prevent minting to address(0)
 ...
 // Blacklist enforced in _update() override
 /// @dev Allow LIQUIDATOR_ROLE to burn for liquidations
 ...
 /// Emergency pause — stops all transfers, mints, and burns
 function pause() external onlyRole(EMERGENCY_ROLE) {
 _pause();
 }

 /// Unpause requires admin (separation of duties)
