...
import { CantonClient, CantonApiError, ActiveContract, TEMPLATES, CIP56_INTERFACES, parseTemplateId } from "./canton-client";
...
interface AttestationRequest {
  ...
  collectedSignatures: { map: [string, Record<string, never>][] };  // Set.Set Party — Canton JSON API v2 encoding
          ...
          if (hasAttestationFields) {
            ...
            try {
              ...
              if (!attestCid) {
                ...
                const validators: string[] = req.payload.validators;
                let signedAttestCid: string | null = null;
                ...
                for (let vi = 0; vi < validators.length; vi++) {
                  ...
                  const selfAttestResult = await this.canton.createContract(
                    TEMPLATES.ValidatorSelfAttestation,
                    {
                  ...
                  if (!selfAttestCid) {
                    const selfContracts = await this.canton.queryContracts(
                      TEMPLATES.ValidatorSelfAttestation,
                      (p: any) => p.attestationId === attestationPayload.attestationId && p.validator === validatorParty
                    ).catch(() => []);
                    if (selfContracts.length > 0) selfAttestCid = selfContracts[selfContracts.length - 1].contractId;
                  }
                  ...
                  if (vi === 0) {
                    ...
                    const signResult = await this.canton.exerciseChoice(
                      TEMPLATES.AttestationRequest,
                      attestCid,
                      "Attestation_Sign",
                      {
                    ...
                    if (!signedAttestCid) {
                      ...
                      const signedContracts = await this.canton.queryContracts(
                        TEMPLATES.SignedAttestation,
                        (p: any) => p.payload?.attestationId === attestationPayload.attestationId
                      ).catch(() => []);
                      if (signedContracts.length > 0) signedAttestCid = signedContracts[signedContracts.length - 1].contractId;
                    }
                  } else {
                    ...
                    const addResult = await this.canton.exerciseChoice(
                      TEMPLATES.SignedAttestation,
                      signedAttestCid,
                    ...
                    if (newCid) signedAttestCid = newCid;
                  }
  ...
  private async processCantonBridgeOuts(): Promise<void> {
    ...
    for (const req of pendingRequests) {
      ...
      try {
        ...
        let bridgeOutTemplate = TEMPLATES.StandaloneBridgeOutRequest;
        try {
          bridgeOutTemplate = parseTemplateId(req.templateId);
        } catch {
