...
async function initCip56Factories() {
  ...
  const existingTransfer = await canton.queryContracts(
    TEMPLATES.MUSDTransferFactory,
    (p: any) => p.admin === cantonParty
  );
  if (existingTransfer.length > 0) {
    console.log(
      `[CIP-56 Init] ✓ MUSDTransferFactory already exists ` +
  ...
  const existingAlloc = await canton.queryContracts(
    TEMPLATES.MUSDAllocationFactory,
    (p: any) => p.admin === cantonParty
  );
  if (existingAlloc.length > 0) {
    console.log(
      `[CIP-56 Init] ✓ MUSDAllocationFactory already exists ` +
  ...
  const cip56Contracts = await canton.queryContracts(undefined, (p: any) => true);
  const cip56Only = cip56Contracts.filter(
    (c: any) => typeof c.templateId === "string" && c.templateId.includes("CIP56Interfaces")
  );
