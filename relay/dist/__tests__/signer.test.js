...
function generateTestSignature() {
    ...
    const messageHash = ethers_1.ethers.keccak256(ethers_1.ethers.toUtf8Bytes("test message"));
 ...
 */
function buildDerFromRSV(rHex, sHex) {
...
describe("formatKMSSignature", () => {
    it("should convert a valid DER signature to RSV format", () => {
        ...
        const rsv = (0, signer_1.formatKMSSignature)(derSig, messageHash, wallet.address);
        expect(rsv).toMatch(/^0x[0-9a-f]{130}$/);
        // Recovered address should match
        const recovered = ethers_1.ethers.recoverAddress(messageHash, rsv);
        expect(recovered.toLowerCase()).toBe(wallet.address.toLowerCase());
    });
    it("should reject a Buffer that is too short", () => {
        expect(() => (0, signer_1.formatKMSSignature)(Buffer.from([0x30, 0x01]), "0xabc", "0xdef")).toThrow("INVALID_DER");
    });
    it("should reject non-Buffer input", () => {
        expect(() => (0, signer_1.formatKMSSignature)("notabuffer", "0xabc", "0xdef")).toThrow("INVALID_INPUT");
    });
    it("should reject missing 0x prefix on messageHash", () => {
        expect(() => (0, signer_1.formatKMSSignature)(Buffer.alloc(70), "abc", "0xdef")).toThrow("INVALID_INPUT: messageHash must be 0x prefixed");
    });
    it("should reject missing 0x prefix on publicKey", () => {
        expect(() => (0, signer_1.formatKMSSignature)(Buffer.alloc(70), "0xabc", "def")).toThrow("INVALID_INPUT: publicKey must be 0x prefixed");
    });
    it("should reject DER with wrong sequence tag", () => {
        const bad = Buffer.alloc(70, 0);
        bad[0] = 0x31; // wrong tag
        expect(() => (0, signer_1.formatKMSSignature)(bad, "0xabc", "0xdef")).toThrow("INVALID_DER: Missing sequence tag");
    });
    it("should reject DER where length exceeds buffer", () => {
        const bad = Buffer.from([0x30, 0xff, 0x02, 0x01, 0x00, 0x02, 0x01, 0x00]);
        expect(() => (0, signer_1.formatKMSSignature)(bad, "0xabc", "0xdef")).toThrow("INVALID_DER");
    });
...
describe("validateSignature", () => {
    it("should return true for a valid signature", () => {
        const wallet = ethers_1.ethers.Wallet.createRandom();
        const hash = ethers_1.ethers.keccak256(ethers_1.ethers.toUtf8Bytes("hello"));
        const sig = wallet.signingKey.sign(hash);
        const rsvSig = sig.serialized;
        expect((0, signer_1.validateSignature)(rsvSig, hash, wallet.address)).toBe(true);
    });
    it("should return false for a wrong signer", () => {
        const wallet = ethers_1.ethers.Wallet.createRandom();
        const other = ethers_1.ethers.Wallet.createRandom();
        const hash = ethers_1.ethers.keccak256(ethers_1.ethers.toUtf8Bytes("hello"));
        const sig = wallet.signingKey.sign(hash).serialized;
        expect((0, signer_1.validateSignature)(sig, hash, other.address)).toBe(false);
    });
    it("should return false for a corrupted signature", () => {
        expect((0, signer_1.validateSignature)("0x" + "00".repeat(65), "0x" + "ab".repeat(32), "0x" + "cd".repeat(20))).toBe(false);
    });
});
// ═══════════════════════════════════════════════
...
describe("sortSignaturesBySignerAddress", () => {
    it("should sort signatures by ascending signer address", () => {
        ...
        const hash = ethers_1.ethers.keccak256(ethers_1.ethers.toUtf8Bytes("sort test"));
        const sigs = wallets.map((w) => w.signingKey.sign(hash).serialized);
        const sorted = (0, signer_1.sortSignaturesBySignerAddress)(sigs, hash);
        // Recover addresses and verify they are sorted
        const recoveredAddrs = sorted.map((s) => ethers_1.ethers.recoverAddress(hash, s).toLowerCase());
    ...
    it("should handle a single signature", () => {
        const wallet = ethers_1.ethers.Wallet.createRandom();
        const hash = ethers_1.ethers.keccak256(ethers_1.ethers.toUtf8Bytes("single"));
        const sig = wallet.signingKey.sign(hash).serialized;
        const sorted = (0, signer_1.sortSignaturesBySignerAddress)([sig], hash);
        expect(sorted).toHaveLength(1);
        expect(sorted[0]).toBe(sig);
    });
    it("should handle empty array", () => {
        const hash = ethers_1.ethers.keccak256(ethers_1.ethers.toUtf8Bytes("empty"));
        const sorted = (0, signer_1.sortSignaturesBySignerAddress)([], hash);
