...
const DEFAULT_CONFIG = {
    ...
    relayerPrivateKey: (0, utils_1.readAndValidatePrivateKey)("relayer_private_key", "RELAYER_PRIVATE_KEY"),
...
class RelayService {
    ...
    constructor(config) {
        ...
        this.wallet = new ethers_1.ethers.Wallet(config.relayerPrivateKey, this.provider);
        this.bridgeContract = new ethers_1.ethers.Contract(config.bridgeContractAddress, BRIDGE_ABI, this.wallet);
    ...
    async pollForAttestations() {
        ...
        let attestations;
        try {
            const queryPromise = this.ledger.query("MintedProtocolV3:AttestationRequest", { aggregator: this.config.cantonParty });
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("Canton query timeout (30s)")), 30000));
    ...
    async fetchValidatorSignatures(requestId) {
        ...
        const signatures = await this.ledger.query("MintedProtocolV3:ValidatorSignature", { requestId });
        return signatures.map(s => s.payload);
    }
    /**
     * Submit attestation to Ethereum
     */
    async bridgeAttestation(payload, validatorSigs) {
        const attestationId = payload.attestationId;
        try {
            ...
            const idBytes32 = ethers_1.ethers.id(attestationId);
            ...
            const attestation = {
                id: idBytes32,
                cantonAssets: ethers_1.ethers.parseUnits(payload.globalCantonAssets, 18),
     ...
     */
    buildMessageHash(payload, idBytes32) {
        ...
        for (const sig of validatorSigs) {
            try {
                ...
                // If signature is DER encoded (from AWS KMS)
                    ...
                    rsvSignature = (0, signer_1.formatKMSSignature)(derBuffer, messageHash, validatorEthAddress // FIX: Use mapped Ethereum address, not DAML Party
                    );
                }
...
function startHealthServer(port, relay) {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if (req.url === "/health") {
            ...
            res.writeHead(200, { "Content-Type": "application/json" });
            res.end(JSON.stringify({
                processedCount: relay.processedAttestations.size,
            }));
        }
        else {
            res.writeHead(404);
