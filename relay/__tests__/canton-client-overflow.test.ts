...
function textErrorResponse(status: number, body: string): JsonResponse {
  return {
    ok: false,
    status,
    json: async () => {
...
function makeActiveEntry(index: number, templateId: string) {
  return {
    contractEntry: {
      JsActiveContract: {
        createdEvent: {
...
function makeClient(): CantonClient {
...
describe("CantonClient queryContracts overflow fallback", () => {
  ...
  it("does not fallback when active-contracts returns 199 entries", async () => {
    ...
    expect((global.fetch as jest.Mock).mock.calls.some((args) => String(args[0]).includes("/v2/updates"))).toBe(false);
  });
